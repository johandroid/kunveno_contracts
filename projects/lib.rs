...
mod projects {
    ...
    impl Project {
        ...
        pub fn mark_completed(&mut self, ratings: Vec<(AccountId, u8)>) -> Result<()> {
            ...
            if let Some(_scope) = &self.scope {
                ...
                remaining_payment = self.total_cost - self.paid_amount;
        ...
        pub fn define_scope(
            ...
            // Check project state
            if self.status != ProjectStatus::TeamAssigned && self.status != ProjectStatus::CoordinatorAssigned {
                return Err(Error::InvalidProjectState);
            }
            ...
            let tasks_vec: Vec<Task> = tasks
                .into_iter()
                .map(|(id, complexity, cost, dependencies)| {
                    Task {
                        id,
                        complexity,
                        cost,
            ...
            let tasks_count = tasks_vec.len() as u8;
        ...
        pub fn accept_scope(&mut self) -> Result<()> {
            ...
            let advance_payment = self.total_cost
    ...
    mod tests {
        ...
        fn assign_team_works() {
            ...
            assert!(team.iter().any(|member| member.account_id == accounts.django));
            assert!(team.iter().any(|member| member.account_id == accounts.frank));
